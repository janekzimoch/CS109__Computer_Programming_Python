...
class RandomWriter(object):
    ...
    def generate(self):
        ...
        self.graph.currentNode = random.choice(list(self.graph.nodes.keys()))

        while True:
            cumulStrength= 0
            randomNumber = random.uniform(0, self.graph.nodes[self.graph.currentNode].totalStrength)

            # it must have edges 
            for edge, strength in self.graph.nodes[self.graph.currentNode].nodes.items(): 
                if edge == None:
                    if len(self.graph.nodes[self.graph.currentNode].nodes) == 1:
                        self.graph.currentNode = random.choice(list(self.graph.nodes.keys()))
                        break
                    else:
                        continue

                prevCumulStrength = cumulStrength
                cumulStrength += strength

                if prevCumulStrength < randomNumber <= cumulStrength:
                    self.graph.goNextNode(edge)
                    yield edge
    ...
    def generate_file(self, filename, amount):
        ...
        mode = self.tokenization
        generator = self.generate()
        # Open file byte mode.
        if mode == Tokenization.byte:
            with open(filename, "wb") as file:
                    while amount:
                        token = next(generator)
                        file.write(bytes([token]))
                        amount -= 1
        
        # Open file text mode.
        else: 
            with open(filename, "w", encoding='utf-8') as file:
                if mode == Tokenization.character:
                    while amount:
                        token = next(generator)
    ...
    def save_pickle(self, filename_or_file_object):
        ...
        """
        with open(filename_or_file_object, "wb") as fileHandler:
    ...
    def load_pickle(cls, filename_or_file_object):
        ...
        """
        with open(filename_or_file_object, 'rb') as fileHandler:
            model = pickle.load(fileHandler)
            if isinstance(model, graph.Node):
                return model
            else:
                raise TypeError('{} does not contain a model'.format(filename_or_file_object))
    ...
    def train_url(self, url):
        ...
        if self.tokenization == Tokenization.byte:
            data = raw_data.content  # reads as bytes
        elif self.tokenization in {Tokenization.character, Tokenization.word}:  # i have to decide what to do when the mode is none
            data = raw_data.text  # reads as a text
        else:
            raise TypeError("non existing tokenization mode was selected for RandomWriter")
    ...
    def train_iterable(self, data):
        ...
        state = None
        for t in data:
            if len(tempNode) < self.level:
                tempNode.append(t)
            else:
                tempNode.pop(0)
                tempNode.append(t)
                if state:
                    self.graph.addNode(state, t)

            if len(tempNode) == self.level:
                state = tuple(tempNode)
